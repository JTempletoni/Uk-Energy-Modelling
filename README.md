...
An end-to-end research pipeline for generating synthetic GB electricity price paths and using them in downstream gas storage stochastic control and deep hedging. Built from free public APIs (Elexon, NESO, Carbon Intensity, LCCC), path signatures, four parallel generative models (CVAE, econometrically-conditioned CVAE, SigCWGAN, neural SDE), and a deep hedging network. The four streams exist so each one fixes a measured failure of the previous one, with a final cross-stream comparison notebook tying the results together.

**Status.** Streams 1 and 2 are complete and validated. Stream 3 (SigCWGAN) is in active development. Stream 4 (neural SDE) is fully specified but not yet trained. Notebooks 01–06, 07a, 07b and 07d are runnable; 07c, 07e, 07f are planned. Each stream will get its own README once it lands; this document is the meta-README that ties them together.

---

## About this project

This is a personal learning project, not a production system. Its purpose is twofold: to build a working synthetic data pipeline on a structurally awkward real dataset, and to use that pipeline as a vehicle for going deep on the areas of mathematics and quantitative finance I want to learn. The writeup is honest about what works, what failed, and why — the diagnosed failure modes of the first model are exactly what motivate the second, third and fourth.

The original motivation was concrete. I applied for a data role at a Scottish energy firm, started looking at the publicly available GB electricity data, and realised that living in the north of Scotland I already had a domain-side view of the market: generation gets built faster than transmission infrastructure, so there are bottlenecks, and a rabbit hole on what happens to excess renewable energy (filling pumped-storage dams for later release) turned into the structural backbone of the project. GB electricity is a deliberate choice as a test bed — free APIs, negative prices, wind cannibalisation, CfD policy, the B6 transmission constraint — the data is genuinely hard to model, which makes it a much better stress test of these methods than equity indices.

The first pipeline was built almost entirely from intuition and from papers I had accumulated studying other things. It worked well enough to generate synthetic paths that pass the primary statistical tests, and it produced clear, diagnosable failures — most importantly, it cannot reproduce inter-segment volatility clustering, which turns out to be an architectural property of the CVAE rather than a tuning bug. That is exactly the kind of failure I wanted: one that points unambiguously at the next thing to build. The redesigned pipeline (stream 2 onwards) starts by studying the energy market as a *system* — discovering its signals, regimes, and exogenous drivers independently of price data — and only then builds a generative model conditioned on what the system analysis surfaces. Stream 2 also brings in the statistical rigour the first pass was missing: proper sampling, look-ahead checks, and regime labels that do not depend on the variable being predicted. Stream 3 adds autoregressive feedback and Hawkes intensities; stream 4 closes the circle with a continuous-time neural SDE.

I am learning as I go, and learning what I need to learn is half the work. I learnt that the CVAE was the wrong final answer the hard way, then went and seriously studied the data, then trained a much better CVAE, then hit the architectural ceiling and moved on. The project is computationally bounded by Colab Pro and ambition-bounded by the fact that I am one person, but the four-stream structure is the genuine plan rather than a wish list.

## Notes on style and process

A few things worth flagging up front, since they recur throughout the notebooks and the writeup:

- **Lots of print statements and plots.** Learning *what* to print, *when*, and which diagnostic plots are actually informative rather than decorative is part of what I am figuring out. The notebooks lean print-heavy on purpose.
- **Diagnosed failures are kept, not hidden.** Stream 1 remains in the repo, untouched and runnable, even though stream 2 supersedes it. The point of the four-stream comparison is that you can see what each architecture buys you and what it costs.
- **Honest about ambition vs delivery.** Where a notebook is planned but not built, or built but not yet validated, the README says so. I would rather flag a gap than paper over one.
...
The first pass through the pipeline was intuition-driven: features chosen on domain reasoning, architecture adapted from Bühler et al. (2020), conditioned on what seemed plausible. It worked well enough to generate synthetic paths that pass the primary statistical tests. It also produced clear, diagnosable failures that motivate the second track. That is an acceptable outcome for a first attempt on a small and structurally unusual dataset. Some of the errors were hard to identify as I created them — jumping between notebooks to alter things and not realising the downstream consequences was a recurring pattern, and one I have explicitly built process around in stream 2.
...
The point of doing this as four streams rather than picking one and committing is that I want measured statements about what each model class buys you and what it costs, rather than asserting one is best on theoretical grounds. Each stream addresses a known weakness of its predecessor, and the comparison notebook `08_stream_comparison` is the final deliverable that ties them together.
