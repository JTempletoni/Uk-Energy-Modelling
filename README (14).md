...
**A small confession.** I described SigCWGAN to myself early on as "a GAN with an LSTM generator and a signature discriminator". Both halves were wrong — it is an AR-FNN, not an LSTM, and there is no discriminator. I caught this by re-reading the paper before writing code, which is the kind of thing I should be doing more of generally.
